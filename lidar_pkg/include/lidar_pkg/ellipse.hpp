// 这个文件主要实现了障碍物椭圆拟合和可视化的功能。

// 输入数据：
// 1. 障碍物点集（ std::vector<Obstacle> ），其中每个 Obstacle 是一组二维点的集合（ std::vector<Eigen::Array2i> ）
// 2. 网格地图（ grid_map::GridMap ）

// 输出数据：
// 1. 椭圆数组（ vector<Ellipse> ），每个椭圆包含以下属性：
//    - semimajor：长轴长度
//    - semiminor：短轴长度
//    - cx, cy：椭圆中心坐标
//    - theta：旋转角度
//    - label：障碍物标签
//    - variance：形状变化方差（用于判断是否为动态障碍物）
// 主要功能：

// 1. get_ellipse_array ：将点集拟合成最小面积椭圆
// 2. ab_variance_calculation ：计算椭圆形状的变化方差
// 3. visEllipse ：将椭圆可视化（在RViz中显示）
//    - 蓝色：静态障碍物
//    - 红色：动态障碍物
//    - 渐变色：不确定状态
